...
def check_output_guardrails(ai_response: str, user_input: str, tool_results: dict = None) -> tuple[bool, str]:
    """
    ...
    # Check 4: Verify AI is not hallucinating options when none exist
    if tool_results:
        flights = tool_results.get("flight_options", [])
        hotels = tool_results.get("hotel_options", [])
        
        # Check for specific hotel/flight mentions when data is empty
        if not flights:
            # Look for patterns suggesting specific flights
            flight_patterns = [
                r'\$\d+.*flight', r'flight.*\$\d+', 
                r'airline.*\$', r'depart.*\d{1,2}:\d{2}',
                r'flight \w{2}\d+', r'(delta|united|american|southwest|jetblue).*flight'
            ]
            if any(re.search(pattern, response_lower) for pattern in flight_patterns):
                return False, " I apologize, but I don't have specific flight options available right now. Please try different dates or destinations, or check directly with airlines."
        
        if not hotels:
            # Look for patterns suggesting specific hotels
            hotel_patterns = [
                r'\$\d+.*night', r'per night.*\$\d+',
                r'hotel.*\$\d+', r'\d+ star.*hotel',
                r'(marriott|hilton|hyatt|sheraton|holiday inn)',
                r'resort.*\$', r'accommodation.*\$\d+'
            ]
            if any(re.search(pattern, response_lower) for pattern in hotel_patterns):
                return False, " I apologize, but I don't have specific hotel options available right now. Please try adjusting your destination or budget, or check booking sites directly."
        
        # If both are empty and AI suggests specific options
        if not flights and not hotels:
            if any(re.search(pattern, response_lower) for pattern in flight_patterns + hotel_patterns):
                return False, " I apologize, but I don't have flight or hotel options available for your search criteria right now. Please try different destinations, dates, or budget ranges."
...
CRITICAL RULES:
- NEVER invent or suggest specific flights, hotels, prices, or airlines that are not in tool_results
- ONLY use data from flight_options and hotel_options arrays in tool_results
- If arrays are empty, clearly state no options are available
- Do NOT make up hotel names, flight numbers, prices, or booking details

RESPONSE INSTRUCTIONS:
1. **Check tool_results first**: 
   - If flight_options is empty: Do NOT suggest any specific flights
   - If hotel_options is empty: Do NOT suggest any specific hotels
   - Only mention what is actually in the data
2. **Check for availability_message**: If present, communicate this to the user clearly
3. **If no options available**: Say "I don't have specific options available right now" and suggest:
   - Trying different dates or destinations
   - Adjusting budget constraints
   - Checking airline/hotel websites directly
4. **If it's a new trip with data**: Provide complete itinerary using ONLY the data provided
5. **If it's a follow-up**: Focus on that specific aspect using available data
6. **If query_type is "off_topic"**: Politely redirect to travel planning

Stay focused, relevant, and helpful. Only discuss travel-related topics. NEVER fabricate travel details.
""")
...
def run_agent(user_text: str, session_id: str = "default") -> str:
    ...
    try:
        ...
        try:
            final_state_dict = json.loads(tool_output["final_state"])
            tool_results = final_state_dict.get("tool_results", {})
        except:
            tool_results = {}
        ...
        # Check output guardrails with tool results
        is_valid, error_msg = check_output_guardrails(final_output, user_text, tool_results)
        if not is_valid:
            final_output = error_msg if error_msg else "I'm here to help with your travel planning. What destination are you interested in?"
